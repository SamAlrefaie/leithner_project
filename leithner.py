...
#    y = input("for adding a flashcard, press Y/y : ")
 #   while  y == 'y' or y == 'Y':
    ...
    boxnumber = 0
    timeofreview = datetime.strptime(
        str(datetime.now()), "%Y-%m-%d %H:%M:%S.%f")
    listofcards = card.flashcard(
        listofcards, word, meaning, boxnumber, timeofreview)
    ...
    for ele in listofcards:
        if CurrentDate > ele[3] and ele[2] != 6:
            ...
            if x == ele[1]:
                if int(ele[2]) == 0:
                    ele[2] = 1
                    ele[3] = CurrentDate + timedelta(seconds=0)
                    print(
                        "Correct!! Now the Flashcard will be moved to next box (box number 1)")
                elif int(ele[2]) == 1:
                    ele[2] = 2
                    ele[3] = CurrentDate + timedelta(seconds=0)
                    print(
                        "Correct!! Now the Flashcard will be moved to next box (box number 2)")
                elif int(ele[2]) == 2:
                    ele[2] = 3
                    ele[3] = CurrentDate + timedelta(seconds=0)
                    print(
                        "Correct!! Now the Flashcard will be moved to next box (box number 3)")
                elif int(ele[2]) == 3:
                    ele[2] = 4
                    ele[3] = CurrentDate + timedelta(seconds=0)
                    print(
                        "Correct!! Now the Flashcard will be moved to next box (box number 4)")
                elif int(ele[2]) == 4:
                    ele[2] = 5
                    ele[3] = CurrentDate + timedelta(seconds=0)
                    print(
                        "Correct!! Now the Flashcard will be moved to next box (box number 5)")
                elif int(ele[2]) == 5:
                    ele[2] = 6
                    ele[3] = CurrentDate + timedelta(seconds=0)
...
def start():
    z = input(
        "Hi ,to add a new word type:' new ' to practice , type ' p '  and to quit type: 'q': ")
    if z == 'new':
        new_word()
    elif z == 'p':
        practice()
